...
var setting = JSON.parse(fs.readFileSync('./app/setting.json'));
module.exports = function(app) {
    ...
    app.get('/video/:v', function(req, res){
        Media.findOne({
            url: req.params.v,
            media: 'video'
        }, function(err, v) {
    ...
    app.get('/danh-muc-video/:i', function(req, res){
        CMedia.findOne({
            url: req.params.i,
            type: 'video'
        }, function(err, c) {
    ...
    app.get('/danh-muc-hinh-anh/:i', function(req, res){
    ...
    app.get('/hinh-anh/:i', function(req, res){
        Media.findOne({
            url: req.params.i,
            media: 'image'
        }, function(err, img) {
    ...
    app.get('/dat-cau-hoi', function(req, res) {
        Promise.props({
            cquestions: CQuest.find().execAsync(),
            questions: Quest.find().execAsync()
        }).then(function(data) {
    ...
    app.get('/dat-cau-hoi/:t', function(req, res){
        CQuest.findOne({url: req.params.t}, function(err, c){
            if(!c) return res.redirect('/');
            Promise.props({
                cquestions: CQuest.find().execAsync(),
                questions: Quest.find({'category._id': c._id}).execAsync()
            }).then(function(data){
                data.cquestion = c;
                return res.render('home/question-category', {
                    q: data,
                    data: req.frame,
                    title: 'Hỏi đáp ' + c.name
                });
            });
        });
        
    })
    app.get('/cau-hoi/:q', function(req, res){
        Quest.findOne({url: req.params.q}, function(err, q){
            if(!q) return res.redirect('/');
            Promise.props({
                question: q,
                cquestions: CQuest.find().execAsync()
            }).then(function(data){
                return res.render('home/question-detail', {
                    q: data,
                    data: req.frame,
                    title: q.name
                });
    ...
    app.get('/bai-viet/:p', function(req, res) {
        var cip = getClientIp();
        Posts.findOne({
            url: req.params.p
        }, function(err, p) {
    ...
    app.get('/:t/:c', function(req, res, next) {
        Cates.findOne({
            url: req.params.c,
            ...
            if (req.query.record) {
                ...
                var record = setting.category.record;
            }
            Posts.find({
    ...
    function selectRecord(i, data, callback) {
        Cates.find().exec(function(err, c) {
            if (c[i]) {
                Posts.find({
                    'category._id': c[i]._id
                }).limit(5).exec(function(err, p) {
                    data.push(p);
                    i++;
    ...
    function asset(req) {
        return req.protocol + '://' + req.headers.host + '/public/';
    }
